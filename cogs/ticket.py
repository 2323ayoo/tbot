...
class ticket_bott(discord.ui.View):
    ...
    @discord.ui.button(label = "Open Ticket To Buy", style = discord.ButtonStyle.grey, custom_id = "ticket_button_1", emoji="<:bot_ticket:1124252501491851335> ")
    async def apriticket(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if calldown_status:
            return await interaction.response.send_message(f"Callmdown! Wait {round(calldown_status, 1)} seconds!", ephemeral = True)
        ...
        if check_ticket_esistente is not None:
            ...
            try:
                ...
                return await interaction.response.send_message("Error Code : >>445>3346<PERMISSION NOT ENOUTH>", ephemeral = True)
...
class confirm(discord.ui.View):
    ...
    async def confirm_button(self, interaction, button):
        ...
        embed.add_field(name="Opened from:", value=f"<@{interaction.channel.topic}>", inline=True)
        ...
        try:
            ...
            print(f"[ERROR Ticket Log] {Fore.LIGHTRED_EX} >>233>65432<-> [{interaction.channel.topic}] {Style.RESET_ALL}")
        ...
        try:
            ...
            await interaction.response.send_message("Error Code : >>445>3346<PERMISSION NOT ENOUTH>", ephemeral = True)
...
class Rimuovi(ui.Modal, title='Remove user'):
        ...
        async def on_submit(self, interaction: discord.Interaction):
            ...
            canalelog = interaction.guild.get_channel(logticket)
            embed=discord.Embed(title="", description=f"**User:** {interaction.user.mention}\n**Status:** Removing user to ticket\n**Channel:** {interaction.channel.mention}\n**User**: {user.mention}", color=embedcolor)
            embed.set_author(name='Log Bot', icon_url=f'{logoticket}')
            embed.set_footer(text=ticketfooterdev, icon_url=f'{logoticket}')
            await canalelog.send(embed=embed)
...
class Ticket(commands.Cog):
    ...
    @commands.hybrid_command(name = 'ticket', with_app_command = True, description='') 
    ...
    async def ticketing(self, ctx):
        if ctx.guild is None:
            ...
            await ctx.send("Naricht Wird Gesendet", ephemeral = True)
            canale = ctx.message.channel
            embed = discord.Embed(title ='Support', description="""**How can we help? /n Welcome to our tickets channel. If you have any questions or inquiries, please click on the  button below to contact the staff!**""", color = embedcolor)
            embed.set_author(name=ticketauthor, icon_url=f"{logoticket}")
            embed.set_footer(text=ticketfooterdev, icon_url=f'{logoticket}')
            await canale.send(embed = embed, view = ticket_bott())
            print(f"[Ticket Log] {Fore.LIGHTGREEN_EX}Ticket created! [@{ctx.message.author} - #{canale}]{Style.RESET_ALL}")
